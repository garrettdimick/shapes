...
public class CircleTest {
    ...
    public void testMove() throws Exception {
        ...
        myCircle.moveCenter(-12,  -26);
        assertEquals(-8, myCircle.getCenter().getX(), 0);
        assertEquals(-20, myCircle.getCenter().getY(), 0);
        assertEquals(5, myCircle.getRadius(), 0);

        try {
            myCircle.moveCenter(Double.POSITIVE_INFINITY, 1);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid x value", e.getMessage());
        }

        try {
            myCircle.moveCenter(Double.NEGATIVE_INFINITY, 1);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid x value", e.getMessage());
            // ignore
        ...
        try {
            myCircle.moveCenter(Double.NaN, 1);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid x value", e.getMessage());
        }
        ...
        try {
            myCircle.moveCenter(1, Double.POSITIVE_INFINITY);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid y value", e.getMessage());
        }

        try {
            myCircle.moveCenter(1, Double.NEGATIVE_INFINITY);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid y value", e.getMessage());
        }
        ...
        try {
            myCircle.moveCenter(1, Double.NaN);
            fail("Expected exception not thrown");
        } catch (ShapeException e) {
            assertEquals("Invalid y value", e.getMessage());
        }
